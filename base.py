...
def getTotalPages(conn, url):
    ...
    try:
        totalPages = math.ceil(json.loads(soup.find("iw-search")[":result-count"])/30)
    except TypeError as Err:
        print("No total page found. Please review the URL (getTotalAds)")
        totalPages = -1
...
def getDataFromTree(searchKey, data):
    ...
    try:
        ...
        print(f'Can\'t extract {searchKey} from {data}')
        return None
